...
- `find-duplicate-logic/SKILL.md`: Find behaviorally duplicated or substantially overlapping repository logic.
- `git-secret-guard/SKILL.md`: Scan for exposed credentials, configure commit protection, and guide leak response. Its consent-bound prerequisite bootstraps and optional Python/Gitleaks helper live under `scripts/`; isolated tests live under `tests/`, and `VALIDATION.md` records evidence and gaps.
...
- Use `skill/<skill-name>` branches for skill work.
...
- Every new skill must include installation and usage guidance for Claude Code, Codex, Cursor, and GitHub Copilot, plus other supported clients when relevant. Cover prerequisites and dependency setup, project versus personal installation, discovery/activation, at least one concrete usage example and expected result, and common setup failures. Explain Windows, macOS, Linux, and remote-environment differences where applicable; state unsupported or untested combinations rather than implying universal support. Keep shared client instructions in `INSTALLATION.md`, with current vendor links and verification dates, and include a sufficient quick start or bundled setup guide inside each skill folder so standalone installs do not depend on `../INSTALLATION.md`. Separate installing the skill from configuring external tools, repository hooks, services, or permissions. Check documentation links and examples before handoff; record live installations separately from documentation review.
...
- For `git-secret-guard`, run `GITLEAKS_TEST_BINARY=/path/to/gitleaks python3 -B -m unittest discover -s git-secret-guard/tests -v`. Tests use temporary repositories and synthetic credentials. Integration tests skip without Gitleaks; report skips rather than claiming scanner validation.
...
- 2026-09-18: Expanded `git-secret-guard` to version 1.2 at stage review with macOS/Linux/WSL and Windows prerequisite bootstraps. They select the smallest dependency set for scan, pre-commit, or pre-push; emit a read-only exact plan; require its plan ID after explicit confirmation; checksum official Gitleaks downloads; and return absolute paths for repository-local hooks without requiring PATH edits. Added a native pre-commit template and aligned standalone/shared installation guidance. All 52 tests passed with Gitleaks 8.30.1, including seven non-installing Linux planner/consent tests and absolute-path hook integration. Structural, metadata/UI, link, Python/POSIX-shell syntax, secret-scan, and whitespace checks passed. No live package installation occurred; Windows/macOS execution, PowerShell parsing, successful downloads, elevation, and package-manager mutations remain untested.
- 2026-09-18: Expanded `git-secret-guard` to version 1.1 at stage review with a standalone four-client/platform setup guide, first-run and contributor instructions, explicit dependency-installation confirmation/notification/verification, and a native pre-push template using full outgoing-tip ancestry with merge diffs. Established installation/usage documentation requirements for every new skill. All 44 tests passed with Gitleaks 8.30.1, including actual local bare-remote pushes for historical leaks, multiple refs/tags, force pushes, merges, deletions, and failure handling. Independent evaluation exercised a fresh contributor clone and identified shared-hook-directory/help-command documentation corrections; dependency consent was scenario-reviewed without live installs. Structural, metadata/UI, local-link, Python/Bash syntax, secret-scan, and whitespace checks passed. Client/package installations, Windows/macOS execution, hook-manager composition, and hosted enforcement remain untested.
- 2026-09-18: Added `git-secret-guard` version 1.0 at stage review on `skill/git-secret-guard`, and documented the `skill/<skill-name>` branch convention. Added staged/files/history scanning guidance, a metadata-only Python/Gitleaks helper, hook/CI setup and leak-response references, UI metadata, standalone GPL license, usage documentation, and validation evidence; updated the catalog and shared installation scope. All 26 isolated tests passed with Gitleaks 8.30.1, including real native hook blocking, partial staging, historical leaks, and failure handling. Independent Codex evaluation confirmed staged detection and preservation; missing-input/incident scenarios and discovery boundaries were assessed manually. Structural, metadata/UI, link, Python syntax, secret-scan, and whitespace checks passed. Other clients/platforms, alternate scanners, the pre-commit framework, and hosted enforcement remain untested.
- 2026-09-18: Added minimal `agents/openai.yaml` interface metadata to `production-readiness-review` and `token-efficiency`, with skill-specific display text, explicit default prompts, Chat/Codex product scope, and implicit invocation enabled. Neither skill needed tool dependencies or dedicated icon assets. Both skills passed the local skill-creator validator, UI metadata assertions, and whitespace checks. Their `SKILL.md` behavior, version, and lifecycle stage are unchanged.
- 2026-09-18: Reviewed and added `find-duplicate-logic` version 1.0 at stage review. Replaced an unsupported frontmatter field with required repository metadata, added author and GPL attribution, tightened missing-input, stop-condition, reference-routing, discovery-boundary, and untracked-diff guidance, and aligned its OpenAI UI metadata and product policy. Documented human-facing use cases and limitations, clarified that the portable `SKILL.md` works in Codex, Claude Code, and Cursor while `agents/openai.yaml` is OpenAI-specific, and refreshed the shared installation scope and dates for those clients. Added the skill to the root catalog. The local skill-creator validator, metadata/UI assertions, local-link and whitespace checks passed. Codex target- and diff-mode fixtures correctly identified exact and semantic duplicates, rejected a different-domain threshold match, treated generated code as low-value, covered an untracked change, and surfaced missing-target and search-scope failures. Automatic invocation and cross-agent runtime behavior remain untested; the standalone `skills-ref` validator was unavailable.
