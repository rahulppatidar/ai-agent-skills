...
- 2026-09-18: Reviewed and added `find-duplicate-logic` version 1.0 at stage review. Replaced an unsupported frontmatter field with required repository metadata, added author and GPL attribution, tightened missing-input, stop-condition, reference-routing, discovery-boundary, and untracked-diff guidance, and aligned its OpenAI UI metadata and product policy. Documented that the portable `SKILL.md` works in Codex, Claude Code, and Cursor while `agents/openai.yaml` is OpenAI-specific; refreshed the shared installation scope and dates for those clients. Added the skill to the root catalog. The local skill-creator validator, metadata/UI assertions, local-link and whitespace checks passed. Codex target- and diff-mode fixtures correctly identified exact and semantic duplicates, rejected a different-domain threshold match, treated generated code as low-value, covered an untracked change, and surfaced missing-target and search-scope failures. Automatic invocation and cross-agent runtime behavior remain untested; the standalone `skills-ref` validator was unavailable.
